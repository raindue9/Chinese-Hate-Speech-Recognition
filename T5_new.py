...
def preprocess_data(examples, tokenizer):
    ...
    with tokenizer.as_target_tokenizer():       # 对目标文本进行编码
    ...
    # 合并结果 将处理好的 labels 加入 model_inputs 中返回
    # 即：
    # 把刚才处理好的目标标签（即 labels）和输入文本对应的 tokenized 结果（input_ids, attention_mask）合在一起，
    # 返回给 Hugging Face 的 Dataset.map() 函数，这样模型训练的时候就知道：
    # “我要输入这些 token，并让模型学会输出这些标签。”
...
if __name__ == "__main__":
    ...
    # 将原始训练样本列表 train_examples 转换成 Hugging Face Dataset 格式
    ...
    tokenized_datasets = raw_datasets.map(
        lambda examples: preprocess_data(examples, tokenizer),      # 分词器处理 生成 input_ids、attention_mask、labels 等字段
        batched=True,
        remove_columns=raw_datasets["train"].column_names           # 把原始的 id、content、output 字段都移除
    )
